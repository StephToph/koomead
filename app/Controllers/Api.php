...
class Api extends BaseController {
	...
	public function production($type='get') {
		...
		if($type == 'get') {
			...
			// query
			$query = $this->Crud->read('production', $limit);
		    if(!empty($query)) {
				$status = true;
				$msg = 'Successful';
				foreach($query as $q) {
		...
		if($type == 'getVideos') {
			// collect call paramters
		    $call = json_decode(file_get_contents("php://input"));
			$videoIds = $call->videoIds;
			if(!empty($this->request->getGet('age_id'))) { $age_id = $this->request->getGet('age_id'); } else { $age_id = 0; }

			if(empty($videoIds)) {
				$msg = 'Please provide at least 1 Video ID, or multiple IDs separated by a comma (,)';
			} else {
				...
				if(!empty($ids)) {
					foreach($ids as $id) {
						...
						if(!empty($age_id)) {
							$query = $this->Crud->read2('id', trim($id), 'age_id', $age_id, 'video');
						} else {
							$query = $this->Crud->read_single('id', trim($id), 'video');
						}
						if(!empty($query)) {
							$status = true;
							$msg = 'Successful';
							foreach($query as $q) {
								$item['id'] = $q->id;
								$item['channel_id'] = $q->production_id;
								$item['channel'] = $this->Crud->read_field('id', $q->production_id, 'production', 'name');
								$item['name'] = $q->title;
				...
				if(empty($data)) $msg = 'No Video Found';
			}
		}
	...
	public function recent($type='get') {
		...
		if($type == 'get') {
			...
			// query
			$query = $this->Crud->read_single_order('user_id', $user_id, 'recent', 'views', 'desc', $limit);
		    if(!empty($query)) {
				$status = true;
				$msg = 'Successful';
				foreach($query as $q) {
	...
	public function release($type='get') {
		...
		if($type == 'get') {
			...
			if(!empty($production_id)) {
				$query = $this->Crud->read_single_order('production_id', $production_id, 'video', 'id', 'desc', $limit);
			} else {
				$query = $this->Crud->read_order('video', 'id', 'desc', $limit);
			}
		    if(!empty($query)) {
				$status = true;
				$msg = 'Successful';
	...
	public function trending($type='get') {
		...
		if($type == 'get') {
			...
			// query
			$query = $this->Crud->read_order('video', 'views', 'desc', $limit);
		    if(!empty($query)) {
				$status = true;
				$msg = 'Successful';
				foreach($query as $q) {
	...
	public function game($type='get') {
		...
		if($type == 'get') {
			...
			// query
			$query = $this->Crud->read_order('game', 'id', 'desc', $limit);
		    if(!empty($query)) {
				$status = true;
				$msg = 'Successful';
				foreach($query as $q) {
