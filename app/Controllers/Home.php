	...
	public function profile($param1='', $param2='', $param3='') {
		...
		if($param1 == 'load') {
			...
			$user_id = $this->request->getVar('user_id');
				...
				$query = $this->Crud->read2('user_id', $user_id, 'active', 1, 'listing', $limit, $offset);
				$all_rec = $this->Crud->read2('user_id', $user_id, 'active', 1, 'listing', '', '');
				if(!empty($all_rec)) { $count = count($all_rec); } else { $count = 0; }
				$role_id = $this->Crud->read_field('id', $log_id, 'user', 'role_id');
				$role = strtolower($this->Crud->read_field('id', $role_id, 'access_role', 'name'));
					...
					if($this->Crud->check2('code', $promo_code, 'status', 0, 'business_promotion') > 0){
						...
						if($expiry_date > date('Y-m-d')){
							if($this->Crud->check2('ip_address', $ipAddress, 'code', $promo_code, 'listing_view') == 0){
								if($this->Crud->check3('user_agent', $user_agent, 'code', $promo_code, 'user_id', $business_id, 'promotion_metric') == 0){
